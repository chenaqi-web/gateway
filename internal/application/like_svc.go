...
func (s *LikeService) BatchLikeStatus(ctx context.Context, userID uint64, req dto.BatchLikeStatusRequest) (*dto.BatchLikeStatusResponse, error) {
	resp, err := s.rpc.LikeClient.BatchLikeStatus(ctx, &likepb.BatchCommentLikeStatusRequest{UserID: userID, ObjectType: req.ObjectType, ObjectIDs: req.ObjectIDs})
	if err != nil {
		return nil, err
	}
